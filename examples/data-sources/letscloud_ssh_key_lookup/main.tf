...
data "letscloud_ssh_key_lookup" "by_id" {
  id = "ssh-key-12345"
}

# Data source to fetch SSH key by label
data "letscloud_ssh_key_lookup" "by_label" {
...
resource "letscloud_instance" "with_existing_key" {
  ...
  ssh_keys = [data.letscloud_ssh_key_lookup.by_label.id]
...
output "ssh_key_by_id" {
  value = {
    id    = data.letscloud_ssh_key_lookup.by_id.id
    label = data.letscloud_ssh_key_lookup.by_id.label
  }
}

output "ssh_key_by_label" {
  value = {
    id    = data.letscloud_ssh_key_lookup.by_label.id
    label = data.letscloud_ssh_key_lookup.by_label.label
  }
}
